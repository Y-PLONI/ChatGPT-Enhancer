// ==UserScript==
// @name         כלי עזר משולבים וסרגל צד ל-ChatGPT (עם הגדרות v3.1.8 - סרגל דינמי מרובה עמודות - מרווח משופר)
// @namespace    http://tampermonkey.net/
// @version      3.1.8
// @description  משלב עיצוב בועות, RTL, העתקה, הסתרת "תוכניות", וסרגל צד Timeline דינמי מרובה עמודות, עם התאמה אישית, אופטימיזציות, ותמיכה במצב כהה.
...
(function() {
    ...
    debugLog('ChatGPT Combined Utilities & Timeline Script v3.1.8 - Started');
    ...
    function findAndHidePlansButton() {
        ...
        if (itemToHide) {
            ...
            if (wrapperToHide && wrapperToHide.tagName !== 'BODY' && wrapperToHide.contains(itemToHide) && wrapperToHide.classList.contains('bg-token-bg-elevated-secondary')) {
                ...
                itemToHide.style.display = 'none'; // Ensure inner is also hidden
            } else {
    ...
    const TIMELINE_SIDEBAR_SPACING_PX = 12;             // MODIFIED: Increased spacing
    const TIMELINE_SIDEBAR_VISUAL_WIDTH_PX = 30;       // MODIFIED: Increased visual width estimate
    ...
    function getTimelineCSS() {
        ...
        return `
            div[id^="${TIMELINE_HTML_ID_PREFIX}"] {
                ...
                width: 4px !important; /* This is the bar itself, text is outside this */
            ...
            div[id^="${TIMELINE_HTML_ID_PREFIX}"] .cgpt-dots li span {
                ...
                left: 16px !important; /* Position number to the right of the dot */
    ...
    function timeline_updateTimelinePositionAndVisibility() {
        ...
        const baseLeftOffset = mainRect.left + 15; // Initial offset from the left of main
        ...
        for (let s = 0; s < numSidebars; s++) {
            ...
            // Calculate left position for this sidebar instance
        ...
        // Hide any unused sidebar DIVs (e.g., if message count dropped)
        ...
        if (bestMatchIndex !== -1 && (timeline_currentMessageIndex !== bestMatchIndex || $allDotsLi.filter('.active').length === 0) ) {
            ...
            if (messagesPerEffectiveSidebar > 0) {
                ...
                const localIndexInSidebar = timeline_currentMessageIndex % messagesPerEffectiveSidebar;
                 if(targetSidebarIndex < numSidebars){ // Ensure sidebar index is valid
    ...
    function main() {
        ...
        GM_registerMenuCommand('הגדרות כלי עזר וסרגל צד (v3.1.8)', openSettingsDialog);
        ...
        debugLog('CombinedScript & Timeline Script v3.1.8 - Fully initialized.');
    }
